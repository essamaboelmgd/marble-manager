...
import type {
  InvoiceDetail as DomainInvoiceDetail,
  InvoiceType,
  PartyRecord,
  ProductRecord,
  SaleLineInput,
} from "./domain/types";
import { calculateAreaQtyScaled, calculateLineTotalMinor } from "./domain/calculations";
import type { MarbleApi } from "./shared/api";
import { filterAndPaginate, normalizeSearch } from "./shared/listing";
...
type InvoiceDetail = DomainInvoiceDetail;
...
function areaQtyScaled(width: string, height: string): number {
  const parsedWidth = Number(width);
  const parsedHeight = Number(height);
  if (!Number.isFinite(parsedWidth) || !Number.isFinite(parsedHeight) || parsedWidth <= 0 || parsedHeight <= 0) return 0;
  return calculateAreaQtyScaled(parsedWidth, parsedHeight);
}

function areaLineTotal(width: string, height: string, price: string): number {
  return calculateLineTotalMinor(areaQtyScaled(width, height), Math.round(Number(price || 0) * 100));
}

function amountInputToMinor(value: string): number {
...
function ListToolbar({ query, onQueryChange, placeholder, totalItems }: { query: string; onQueryChange: (value: string) => void; placeholder: string; totalItems: number }) {
  return <div className="list-toolbar"><label className="search-field"><Search size={16} /><span className="sr-only">بحث</span><input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder={placeholder} aria-label={placeholder} /></label><span className="result-count">{totalItems} نتيجة</span></div>;
}

function Pagination({ currentPage, totalPages, onChange }: { currentPage: number; totalPages: number; onChange: (page: number) => void }) {
  if (totalPages <= 1) return null;
  return <div className="pagination"><button className="page-button" disabled={currentPage <= 1} onClick={() => onChange(currentPage - 1)} aria-label="الصفحة السابقة"><ChevronRight size={16} /> السابقة</button><span>صفحة {currentPage} من {totalPages}</span><button className="page-button" disabled={currentPage >= totalPages} onClick={() => onChange(currentPage + 1)} aria-label="الصفحة التالية">التالية <ChevronLeft size={16} /></button></div>;
}
...
function CustomersPage({ refreshToken, onChanged }: { refreshToken: number; onChanged: () => void }) {
  ...
  const pagedCustomers = filterAndPaginate(customers, query, (customer, normalizedQuery) => normalizeSearch(`${customer.name} ${customer.phone} ${customer.address}`).includes(normalizedQuery), page);
  ...
  return <>
    ...
    <div className="split-layout">
      ...
      <section className="panel panel-large"><div className="panel-heading"><div><h3>دليل العملاء</h3><p>{customers.length} عميل مسجل</p></div><Users size={20} /></div><ListToolbar query={query} onQueryChange={setQuery} placeholder="ابحث بالاسم أو رقم الهاتف" totalItems={pagedCustomers.totalItems} />{pagedCustomers.totalItems === 0 ? <EmptyState text={query ? "لا توجد نتائج مطابقة" : "لم تضف عملاء بعد"} /> : <><div className="table-wrap"><table><thead><tr><th>الاسم</th><th>الهاتف</th><th>العنوان</th><th>إجراء</th></tr></thead><tbody>{pagedCustomers.items.map((customer) => <tr key={customer.id}><td className="strong-cell">{customer.name}</td><td>{customer.phone || "—"}</td><td>{customer.address || "—"}</td><td><button className="text-button" onClick={() => openStatement(customer.id)}>كشف الحساب <ArrowDownLeft size={14} /></button></td></tr>)}</tbody></table></div><Pagination currentPage={pagedCustomers.currentPage} totalPages={pagedCustomers.totalPages} onChange={setPage} /></>}</section>
    </div>
...
type SaleLine = { kind: "stock" | "extra"; productId: string; name: string; width: string; height: string; price: string };

const newPurchaseLine = (): PurchaseLine => ({ productId: "", width: "", height: "", price: "" });
const newStockLine = (): SaleLine => ({ kind: "stock", productId: "", name: "", width: "", height: "", price: "" });
const newExtraLine = (): SaleLine => ({ kind: "extra", productId: "", name: "", width: "", height: "", price: "" });
...
function updateSaleLine(lines: SaleLine[], setLines: (value: SaleLine[]) => void, index: number, key: keyof SaleLine, value: string) { const next = [...lines]; next[index] = { ...next[index], [key]: value } as SaleLine; setLines(next); }

function LineHeaders({ sale }: { sale?: boolean }) {
...
function PurchasesPage({ refreshToken, onChanged }: { refreshToken: number; onChanged: () => void }) {
  ...
  useEffect(() => { load(); }, [refreshToken]);
  useEffect(() => { setPage(1); }, [query]);
  const pagedInvoices = filterAndPaginate(invoices, query, (invoice, normalizedQuery) => normalizeSearch(`${invoice.invoiceNumber} ${invoice.supplierName} ${invoice.supplierPhone}`).includes(normalizedQuery), page);
  const total = lines.reduce((sum, line) => sum + areaLineTotal(line.width, line.height, line.price), 0);
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      await window.marbleApi.purchases.create({ supplierId: form.supplierId, accountId: form.accountId, method: form.method as "cash" | "bank" | "transfer" | "card" | "cheque" | "other", paidMinor, items: validLines.map((line) => ({ productId: line.productId, width: Number(line.width), height: Number(line.height), qtyScaled: areaQtyScaled(line.width, line.height), unitPriceMinor: Math.round(Number(line.price) * 100) })) });
      toast.success("تم حفظ فاتورة الشراء وتحديث المخزن");
      setForm({ supplierId: "", paid: "", accountId: "account-cash-main", method: "cash" });
...
function SalesPage({ refreshToken, onChanged }: { refreshToken: number; onChanged: () => void }) {
  ...
  useEffect(() => { load(); }, [refreshToken]);
  useEffect(() => { setPage(1); }, [query]);
  const pagedInvoices = filterAndPaginate(invoices, query, (invoice, normalizedQuery) => normalizeSearch(`${invoice.invoiceNumber} ${invoice.customerName} ${invoice.customerPhone}`).includes(normalizedQuery), page);
  const total = lines.reduce((sum, line) => sum + (line.kind === "stock" ? areaLineTotal(line.width, line.height, line.price) : amountInputToMinor(line.price)), 0);
  const paidMinor = amountInputToMinor(form.paid);
  const remainingMinor = Math.max(0, total - paidMinor);
...
function InvoiceTotals({ totalMinor, paidMinor, remainingMinor }: { totalMinor: number; paidMinor: number; remainingMinor: number }) {
  return <div className="invoice-totals"><div><span>الإجمالي</span><strong>{money(totalMinor)}</strong></div><div><span>المدفوع الآن</span><strong className="positive-text">{money(paidMinor)}</strong></div><div><span>المتبقي</span><strong className={remainingMinor > 0 ? "negative-text" : "positive-text"}>{money(remainingMinor)}</strong></div></div>;
}

function InvoiceTable({ type, rows, onDetails, onCancel }: { type: "sale" | "purchase"; rows: Array<SaleInvoice | PurchaseInvoice>; onDetails?: (id: string) => void; onCancel?: (id: string) => void }) {
  if (!rows.length) return <EmptyState text="لا توجد فواتير بعد" />;
  return <div className="table-wrap"><table><thead><tr><th>الرقم</th><th>{type === "sale" ? "العميل" : "المورد"}</th><th>التاريخ</th><th>الإجمالي</th><th>المدفوع</th><th>المتبقي</th><th>الحالة</th>{(onDetails || onCancel) && <th>إجراء</th>}</tr></thead><tbody>{rows.map((row) => { const partyName = type === "sale" ? (row as SaleInvoice).customerName : (row as PurchaseInvoice).supplierName; return <tr key={row.id}><td>#{row.invoiceNumber}</td><td className="strong-cell">{partyName}</td><td>{dateLabel(row.date)}</td><td>{money(row.totalMinor)}</td><td className="positive-text">{money(row.paidMinor)}</td><td className="negative-text">{money(row.remainingMinor)}</td><td><StatusBadge value={row.status} /></td>{(onDetails || onCancel) && <td><div className="table-actions">{onDetails && <button className="text-button" onClick={() => onDetails(row.id)}><Eye size={14} /> التفاصيل</button>}{onCancel && row.status === "open" && <button className="text-button danger" onClick={() => onCancel(row.id)}>إلغاء</button>}</div></td>}</tr>; })}</tbody></table></div>;
}

function InvoiceDetailsModal({ invoiceType, invoiceId, onClose, onChanged }: { invoiceType: InvoiceType; invoiceId: string | null; onClose: () => void; onChanged: () => void | Promise<void> }) {
  const [detail, setDetail] = useState<InvoiceDetail | null>(null);
  const [accounts, setAccounts] = useState<Account[]>([]);
  const [payment, setPayment] = useState({ amount: "", accountId: "account-cash-main", method: "cash" });
  const [loading, setLoading] = useState(false);
  const load = async () => {
    if (!invoiceId) return;
    setLoading(true);
    try { const [nextDetail, accountItems] = await Promise.all([window.marbleApi.invoices.details(invoiceType, invoiceId), window.marbleApi.finance.accounts()]); setDetail(nextDetail); setAccounts(accountItems as Account[]); if (!payment.accountId && accountItems[0]) setPayment((current) => ({ ...current, accountId: (accountItems[0] as Account).id })); } catch (error) { toast.error(errorMessage(error)); } finally { setLoading(false); }
  };
  useEffect(() => { setDetail(null); if (invoiceId) void load(); }, [invoiceId, invoiceType]);
  if (!invoiceId) return null;
  const savePayment = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!detail) return;
    const amountMinor = amountInputToMinor(payment.amount);
    if (!amountMinor || amountMinor > detail.remainingMinor) { toast.error("قيمة الدفعة أكبر من المتبقي أو غير صحيحة"); return; }
    try { await window.marbleApi.payments.create({ invoiceType, invoiceId, accountId: payment.accountId, amountMinor, method: payment.method as "cash" | "bank" | "transfer" | "card" | "cheque" | "other" }); toast.success(invoiceType === "sale" ? "تم تسجيل تحصيل العميل" : "تم تسجيل دفعة المورد"); setPayment({ ...payment, amount: "" }); await load(); await onChanged(); } catch (error) { toast.error(errorMessage(error)); }
  };
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="invoice-details-title"><div className="modal-heading"><div><h2 id="invoice-details-title">تفاصيل الفاتورة #{detail?.invoiceNumber || ""}</h2>{detail && <p>{detail.partyName} {detail.partyPhone ? `· ${detail.partyPhone}` : ""} · {dateLabel(detail.date)}</p>}</div><button className="icon-button" onClick={onClose} aria-label="إغلاق"><X size={18} /></button></div>{loading && !detail ? <div className="modal-loading">جارٍ تحميل التفاصيل...</div> : detail ? <><div className="detail-summary"><div><span>الإجمالي</span><strong>{money(detail.totalMinor)}</strong></div><div><span>المدفوع</span><strong className="positive-text">{money(detail.paidMinor)}</strong></div><div><span>المتبقي</span><strong className={detail.remainingMinor > 0 ? "negative-text" : "positive-text"}>{money(detail.remainingMinor)}</strong></div></div><div className="table-wrap"><table><thead><tr><th>البند</th><th>الأبعاد</th><th>الكمية م²</th><th>السعر</th><th>الإجمالي</th></tr></thead><tbody>{detail.items.map((item) => <tr key={item.id}><td className="strong-cell">{item.name}{item.kind === "extra" && <span className="detail-kind">إضافي</span>}</td><td>{item.width && item.height ? `${item.width} × ${item.height} م` : "—"}</td><td>{quantity(item.qtyScaled)}</td><td>{money(item.unitPriceMinor)}</td><td>{money(item.totalMinor)}</td></tr>)}</tbody></table></div><section className="payment-history"><h3>الدفعات المسجلة</h3>{detail.payments.length ? <div className="table-wrap"><table><thead><tr><th>التاريخ</th><th>الحساب</th><th>الطريقة</th><th>المبلغ</th></tr></thead><tbody>{detail.payments.map((item) => <tr key={item.id}><td>{dateLabel(item.date)}</td><td>{item.accountName}</td><td>{item.method}</td><td className="positive-text">{money(item.amountMinor)}</td></tr>)}</tbody></table></div> : <p className="muted">لا توجد دفعات مسجلة بعد.</p>}</section>{detail.remainingMinor > 0 && detail.status === "open" && <form className="payment-box" onSubmit={savePayment}><h3>{invoiceType === "sale" ? "تحصيل دفعة من العميل" : "تسجيل دفعة للمورد"}</h3><div className="form-grid"><Field label="المبلغ"><input type="number" min="0" step="0.01" value={payment.amount} onChange={(event) => setPayment({ ...payment, amount: event.target.value })} /></Field><Field label="الحساب"><select value={payment.accountId} onChange={(event) => setPayment({ ...payment, accountId: event.target.value })}>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Field></div><Field label="طريقة الدفع"><select value={payment.method} onChange={(event) => setPayment({ ...payment, method: event.target.value })}><option value="cash">نقدي</option><option value="transfer">تحويل</option><option value="card">بطاقة</option><option value="cheque">شيك</option></select></Field><button className="primary-button" type="submit">تسجيل الدفعة</button></form>}</> : null}</section></div>;
}
...
function InvoicesPage({ refreshToken, onChanged }: { refreshToken: number; onChanged: () => void }) {
  ...
  const activeRows = tab === "sales" ? sales : purchases;
  const pagedRows = filterAndPaginate<SaleInvoice | PurchaseInvoice>([...activeRows], query, (invoice, normalizedQuery) => normalizeSearch(`${invoice.invoiceNumber} ${"customerName" in invoice ? invoice.customerName : invoice.supplierName} ${"customerPhone" in invoice ? invoice.customerPhone : invoice.supplierPhone}`).includes(normalizedQuery), page);
  return <><PageIntro title="الفواتير" description="أرشيف المبيعات والمشتريات مع تفاصيل كل بند والدفعات." /><section className="panel"><div className="panel-heading"><div><h3>{tab === "sales" ? "فواتير المبيعات" : "فواتير المشتريات"}</h3><p>{activeRows.length} فاتورة</p></div>{tab === "sales" ? <Receipt size={20} /> : <ShoppingBag size={20} />}</div><div className="tabs" role="tablist"><button className={`tab-button ${tab === "sales" ? "active" : ""}`} onClick={() => setTab("sales")} role="tab" aria-selected={tab === "sales"}>المبيعات</button><button className={`tab-button ${tab === "purchases" ? "active" : ""}`} onClick={() => setTab("purchases")} role="tab" aria-selected={tab === "purchases"}>المشتريات</button></div><ListToolbar query={query} onQueryChange={setQuery} placeholder={tab === "sales" ? "ابحث برقم الفاتورة أو اسم/هاتف العميل" : "ابحث برقم الفاتورة أو اسم/هاتف المورد"} totalItems={pagedRows.totalItems} />{pagedRows.totalItems === 0 ? <EmptyState text={query ? "لا توجد نتائج مطابقة" : "لا توجد فواتير بعد"} /> : <><InvoiceTable type={tab === "sales" ? "sale" : "purchase"} rows={pagedRows.items} onDetails={setDetailId} /><Pagination currentPage={pagedRows.currentPage} totalPages={pagedRows.totalPages} onChange={setPage} /></>}</section><InvoiceDetailsModal invoiceType={tab === "sales" ? "sale" : "purchase"} invoiceId={detailId} onClose={() => setDetailId(null)} onChanged={async () => { onChanged(); }} /></>;
}
...
function AppointmentsPage({ refreshToken, onChanged }: { refreshToken: number; onChanged: () => void }) {
  ...
  useEffect(() => { load(); }, [refreshToken]);
  useEffect(() => { setPage(1); }, [query]);
  const pagedAppointments = filterAndPaginate(appointments, query, (appointment, normalizedQuery) => normalizeSearch(`${appointment.customerName} ${appointment.customerPhone} ${appointment.invoiceNumber}`).includes(normalizedQuery), page);
  ...
  return <><PageIntro title="المواعيد" description="كل مواعيد التركيب المرتبطة بعمليات البيع." /><section className="panel"><div className="panel-heading"><div><h3>تقويم التشغيل</h3><p>{appointments.length} موعد</p></div><CalendarDays size={20} /></div><ListToolbar query={query} onQueryChange={setQuery} placeholder="ابحث باسم العميل أو الهاتف أو رقم الفاتورة" totalItems={pagedAppointments.totalItems} />{pagedAppointments.totalItems === 0 ? <EmptyState text={query ? "لا توجد نتائج مطابقة" : "لا توجد مواعيد مسجلة"} /> : <><div className="appointment-grid">{pagedAppointments.items.map((appointment) => <div className="appointment-card" key={appointment.id}><div className="appointment-date"><span>{new Date(appointment.scheduledAt).toLocaleDateString("ar-EG", { weekday: "long" })}</span><strong>{new Date(appointment.scheduledAt).getDate()}</strong></div><div className="row-main"><strong>{appointment.customerName}</strong><span>{appointment.customerPhone || "بدون هاتف"} · فاتورة #{appointment.invoiceNumber} · {dateLabel(appointment.scheduledAt)}</span><select value={appointment.status} onChange={(event) => update(appointment.id, event.target.value)}><option value="scheduled">مجدول</option><option value="confirmed">مؤكد</option><option value="completed">منفذ</option><option value="cancelled">ملغى</option></select></div></div>)}</div><Pagination currentPage={pagedAppointments.currentPage} totalPages={pagedAppointments.totalPages} onChange={setPage} /></>}</section></>;
}
